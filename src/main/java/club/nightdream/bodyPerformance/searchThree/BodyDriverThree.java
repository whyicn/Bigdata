...
/**
 * this search demonstrate ascending data of each classes
 * */

public class BodyDriverThree {
    public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
        //1. 获取job
        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf);
        //2. 设置jar
        job.setJarByClass(BodyDriverThree.class);
        //3. 关联map和Reducer
        job.setMapperClass(BodyMapperThree.class);
        job.setReducerClass(bodyreducerThree.class);
